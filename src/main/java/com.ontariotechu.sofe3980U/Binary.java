...
public class Binary
{
	...
	*/
	...
	public static Binary multi(Binary num1, Binary num2) {
		int shift=0;
		String sum="0";
		...
		for (int i = num2.number.length()-1; i>=0; i--) {
			if (num2.number.charAt(i)=='1') {
				...
				sum = Binary.add(new Binary(sum), new Binary(temp)).getValue();
			}
		shift++;
		}
		
		Binary ans=new Binary(sum);
		return ans;
	}
}	